...
namespace Web2023Project.Controllers
{
	public class HomeController : PhoneController
	{
		...
		public async Task<ActionResult> Index()
		{
			...
			{
				List<Chitietgiohang> ghss = (List<Chitietgiohang>)Session["giohangss"];
				gh = await _apiService.GetAsync<List<Chitietgiohang>>($"Chitietgiohangs/Nguoidung/{nd.Id}");
				if (ghss != null && ghss.Count > 0)
				{
					foreach (Chitietgiohang i in ghss)
					{
						bool check = true;
						if (gh != null)
						{
							if (i.checkContain(gh))
							{
								foreach (Chitietgiohang j in gh)
								{
									if (i.IdSp == j.IdSp)
									{
										j.Soluong = i.Soluong;
						...
						{
							gh = new List<Chitietgiohang>();
						}
						if (check)
						{
							gh.Add(i);
						}
						Chitietgiohang item = new Chitietgiohang(nd.Id, i.IdSp, i.Soluong, 1);
						await _apiService.PostAsync<Chitietgiohang>("Chitietgiohangs", item);
					}
				}
				Session.Remove("giohangss");
			}
		...
		[HttpPost]
		public async Task<ActionResult> Cart_Process()
		{
			string id = Request["id"];
			int soluong = 1;
			string update = Request["update"];
			Nguoidung nguoidung = (Nguoidung)Session["memberLogin"];
			if (nguoidung == null) // not login - giohangss
			{
				if (id != null)
				{
					int msp = Int32.Parse(id.Trim());
					Sanpham sp = await _apiService.GetAsync<Sanpham>($"Sanphams/{msp}");
					if (sp != null)
					{
						List<Chitietgiohang> gh = (List<Chitietgiohang>)Session["giohangss"];
						if (Request["soluong"] != null)
						{
							...
							{
								gh = new List<Chitietgiohang>();
								Chitietgiohang item = new Chitietgiohang(0, sp.Id, soluong, 1);
								item.IdSpNavigation = sp;
								gh.Add(item);
							}
							else
							{
								bool check = false;
								foreach (Chitietgiohang i in gh)
								{
									if (i.IdSp == sp.Id)
									{
						...
						Session.Add("giohangss", gh);
					}
				}
			}
			else
			{
				int idNd = nguoidung.Id;
				List<Chitietgiohang> gh = await _apiService.GetAsync<List<Chitietgiohang>>($"Chitietgiohangs/Nguoidung/{idNd}");
				if (id != null)
				{
					int msp = Int32.Parse(id.Trim());
					Sanpham sp = await _apiService.GetAsync<Sanpham>($"Sanphams/{msp}");
					...
					if (sp != null)
					{
						//List<Chitietgiohang> gh = (List<Chitietgiohang>)Session["giohang"];
						if (Request["soluong"] != null)
						{
							soluong = Convert.ToInt32(Request["soluong"]);
							...
							{
								gh = new List<Chitietgiohang>();
								Chitietgiohang item = new Chitietgiohang(idNd, sp.Id, soluong, 1);
								gh.Add(item);
								await _apiService.PostAsync<Chitietgiohang>("Chitietgiohangs", item);
							}
							else
						...
						else
						{
							...
							foreach (Chitietgiohang item in gh)
							{
								if (item.IdSp == sp.Id)
								{
									gh.Remove(item);
									item.Trangthai = 0;
									await _apiService.PutAsync<Chitietgiohang>($"Chitietgiohangs/{item.Id}", item);
									break;
								}
							}
						}
						...
						gh = await _apiService.GetAsync<List<Chitietgiohang>>($"Chitietgiohangs/Nguoidung/{idNd}");
						Session.Add("giohang", gh);
		...
		public async Task<ActionResult> PaymentController()
		{
			List<Chitietgiohang> gh = Session["giohang"] as List<Chitietgiohang>;
			Nguoidung member = Session["memberLogin"] as Nguoidung;
			double? total = 0;
			if (gh != null)
			{
				foreach (Chitietgiohang ch in gh)
				{
					total += ch.Soluong * ch.IdSpNavigation.Giadagiam;
				}
				Donhang donhang = new Donhang(member.Id, 1, total, 1);
				await _apiService.PostAsync<Donhang>("DatHang", donhang);
