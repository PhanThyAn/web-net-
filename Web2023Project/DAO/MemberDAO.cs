...
//Admin, User
        public static bool AddMember(Member member)
        {
            ...
            {
                String sql =
                    "INSERT INTO THANHVIEN(taikhoan, matkhau, hoten, gioitinh, email, sodienthoai, diachi, level, avatar) VALUES (@taikhoan,@matkhau,@ten,@gioitinh,@email,@phone,@diachi,@level,@avatar)";
                connection = DBConnection.getConnection();
                connection.Open();
                cmd = new MySqlCommand(sql, connection);
